...
def parse_output(output_stats):
    os.makedirs(os.path.dirname("outputs/file_size.json"), exist_ok=True)
    stats = open("outputs/file_size.json", 'a+')
...
def collect_results():
    data = []
    filename = "outputs/file_size.json"
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    f = open(filename)
    for line in f:
        data.append(json.loads(line.strip()))
    df = pd.DataFrame(data=data)
    df.to_csv("outputs/file_size.csv", index=False)
    os.system("rm outputs/file_size.json")


def read_csv(csv_name, single_col=None):
    begin = time.time()
    # NOTION: please pay attention to sep
    df = pd.read_csv(csv_name, sep=',', header=None, skiprows=1, on_bad_lines='skip', low_memory=False)
