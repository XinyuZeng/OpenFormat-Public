...
def main():
	table = pq.read_table('pq/core_int_256K.pq')
	# Sweep row group size
	for row_group_size in row_group_sizes:
		pq.write_table(table, 'pq/pq_para_row_group_%d.pq' % row_group_size,
			compression='SNAPPY',
			use_dictionary=True,
		...
		}, 'pq_para_row_group_%d' % row_group_size)
		
	# Sweep page size
	for page_size in page_sizes:
		pq.write_table(table, 'pq/pq_para_page_%d.pq' % page_size,
			compression='SNAPPY',
			use_dictionary=True,
			dictionary_pagesize_limit=1048576,
		...
		save_pq_setting(table, {
			'row_group_size': default_row_group_size,
			'page_size': page_size
		}, 'pq_para_page_%d' % page_size)
		
	pq.write_table(table, 'pq/pq_para_default.pq',
		compression='SNAPPY',
		use_dictionary=True,
		dictionary_pagesize_limit=1048576,
		row_group_size=default_row_group_size,
