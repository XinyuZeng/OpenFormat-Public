...
def read_parquet_raw(file_name, exp_stats):
    begin = time.time()
    os.system(
        f"perf record -F 997 -g -e cpu-clock /root/arrow-private/cpp/out/build/openformat-release/release/raw_reader_v2 {file_name} > tmp.txt")
    # os.system(
    #     f"perf record -F 997 -g -e cpu-clock /root/arrow-private/cpp/out/build/openformat-debug/debug/raw_reader_v2 {file_name} > tmp.txt")
    # print("read pq time (s):", time.time() - begin)
    tmp_file = open("tmp.txt", "r")
    output = tmp_file.readlines()[0]
    exp_stats["read_pq_time (s)"] = output.split(':')[-1].strip()
    print("read pq time (s):", exp_stats["read_pq_time (s)"])
...
if __name__ == "__main__":
    ...
    for i, exp in enumerate(exps):
        ...
        print(param_string)
        orc_name = "./{}_{}.parquet".format(name, param_string)
        os.system('sync; echo 3 > /proc/sys/vm/drop_caches # drop mem cache')
        time.sleep(3)
        output_stats = exp.copy()
        output_stats = read_parquet_raw(orc_name, output_stats.copy())
        os.system(
            f'perf script | stackcollapse-perf.pl | flamegraph.pl > outputs/pq_{name}_{param_string}.svg')
        os.system(
            f'perf report --call-graph --stdio -G > outputs/pq_{name}_{param_string}.perfreport')
        # output_stats["file_size (MB)"] = os.path.getsize(orc_name)/1024/1024
        parse_output(output_stats)
    collect_results()
    os.system(f'mv outputs/stats.csv outputs/pq_{name}_{exp_name}.csv')
